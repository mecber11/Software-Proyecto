...
def create_mcnemar_matrix_plot(mcnemar_results):
    ...
    try:
        ...
        significance_matrix = np.zeros((n_models, n_models))  # Diagonal = 0 (no significativo)
        ...
        for result in mcnemar_results:
            ...
            # Solo llenar el triángulo superior (evitar duplicados)
        ...
        for i in range(n_models):
            ...
            for j in range(n_models):
                if i == j:
                    text = f"{model_list[i]}"
                elif i < j:  # Solo mostrar el triángulo superior
                    p_val = pvalue_matrix[i, j]
                    sig_text = "Sig" if significance_matrix[i, j] else "NS"
                    text = f"p: {p_val:.4f}<br>{sig_text}"
                else:  # Triángulo inferior vacío
                    text = ""
        ...
        fig.update_layout(
            title="Matriz de McNemar - Comparaciones por Pares<br><sub>Bordes rojos: diferencias significativas (p<0.05)</sub>",
            width=600,
            height=600,
        ...
        if mcnemar_results or mcc_scores:
            ...
            with tab2:
                ...
                if mcnemar_results:
                    ...
                    df_mcnemar = pd.DataFrame([
                        {
                            ...
                            'Estadístico χ²': f"{result['statistic']:.4f}",
                            'p-valor': f"{result['p_value']:.4f}",
                            'Significativo': "Sí" if result['significant'] else "No",
                            'Interpretación': "Modelos diferentes" if result['significant'] else "Sin diferencia significativa"
                        }
                        for result in mcnemar_results
                    ])
                    ...
                    # Explicación
                    st.info("💡 **Interpretación**: p < 0.05 indica que existe diferencia significativa entre los modelos comparados.")
                else:
